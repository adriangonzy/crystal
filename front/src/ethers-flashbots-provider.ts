...
export class FlashbotsBundleProvider extends providers.JsonRpcProvider {
    ...
    const bundleGasPricing = bundleTransactions.reduce(
      (acc, transactionDetail) => {
        ...
        const gasPricePaidBySearcher = BigNumber.from(
          'gas_price' in transactionDetail
            ? transactionDetail.gas_price
        ...
        return {
          ...
          gasFeesPaidBySearcher: acc.gasFeesPaidBySearcher.add(
            gasPricePaidBySearcher.mul(gasUsed)
          ),
          priorityFeesReceivedByMiner: acc.priorityFeesReceivedByMiner.add(
            priorityFeeReceivedByMiner.mul(gasUsed)
          ),
  ...
  private async request(request: string) {
    ...
    console.log('ratillas', connectionInfo)
    const resp = await fetch(connectionInfo.url, {
      method: 'POST',
      mode: 'cors',
      cache: 'no-cache',
      credentials: 'omit',
